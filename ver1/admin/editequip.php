 <?
	  ...
	  $stmt->execute();
 	  $data=$stmt->fetch();
                ...
                <div class="x_panel">
                  ...
                  <div class="form-group">
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-12">
                          <span><?=$data['inviequip_date']?></span>
                        </div>
                  </div>
                   <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="his-topic">สถานที่ที่ยืม</label>
                        <div class="col-md-2 col-sm-3 col-xs-12">
                          <span><?=$data['inviequip_place']?></span>
                        </div>
                      </div>
					  <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="his-topic">จำนวนที่ยืม</label>
                        <div class="col-md-2 col-sm-3 col-xs-12">
                          <span><?=$data['inviequip_num']?></span>
                        </div>
                      </div>
        ...
        <?
	          if($_POST){
	        $id = $_GET['inviequip_id'];
	        $status = $_POST['status'];

	        $sql = "UPDATE inviequip_tb SET inviequip_status = :status WHERE inviequip_id = :id";
	      	$stmt=$db->prepare($sql);
		  	$param[':id']=$id;
			$param[':status'] = $status;
			$stmt->execute($param);
